...
if length(LOS_points.slicesAv) ~= 1
    if strcmpi(input.flag_probe_weighting,"mean")
        VFinalTotal_Time = mean(VFinalTotal_TimeInt2,'omitnan');% Averaging columns which contain all the volume averaging ppins in the LOS
...
%             weights = linspace (-length(VFinalTotal_TimeInt_noNAN),length(VFinalTotal_TimeInt_noNAN),size(VFinalTotal_TimeInt_noNAN,1));
            ...
            distan         = linspace(-50+focus_distance-input.distance_av_space,50+focus_distance+input.distance_av_space,1e5);
