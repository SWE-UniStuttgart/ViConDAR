...
function writeTurbSimTimeSeriesInput(input_file, data_file, uvw, t, y, z, nComp,RefNode)
...
fprintf(fid,' %10.0f  nComp    - Number of velocity components in the file (1=u component only; 2=u & v components; 3=u,v,w) [if < 3 other components will be generated using values from input file]%s', nComp, endline);
...
elseif nComp==2
    for iz=1:length(z)
        fprintf (fid, 'Point%02.0f%s_%03.0fm ', iz, 'u', z(iz), iz, 'v', z(iz));
    end
    fprintf( fid, '%s', endline);
    fprintf(fid,'          (s)     ' );
    for iz=1:length(z)
...
elseif nComp==1
    for iz=1:length(z)
        fprintf (fid, 'Point%02.0f%s_%03.0fm ', iz, 'u', z(iz));
    end
    fprintf( fid, '%s', endline);
    fprintf(fid,'          (s)     ' );
    for iz=1:length(z)
