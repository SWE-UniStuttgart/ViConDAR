...
input.points_av_slice   = [13]; % How many point/slices you want to take in the averaging of distance_av_slice  Totalpoints = distance_av_slice/points_av_slice+1 IT HAS TO BE AN EXACT DIVISION FOR NOW!!!!
input.sample_rate   = [5.75]; % [Hz] Lidar measurements sample rate (along with the probe length). Sample rate, transformed in distance,  "(1/sample_rate/dt)*distance_slices", must be smaller than the probe volume
