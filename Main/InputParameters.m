...
input.nameBase = 'FLTEST'; %This is the first part of the name to be used for the concatenation
...
input.flag_calculate_fullWF_statistics = 0; % Calculate statistics from full wind fields (constrained and/or original). Code looks in all folders using the names provided as input
input.flag_floating                    = 1; % Consider Floating Dynamics of WT, Dynamics files from Aeroelastic simulation must be available
input.flag_saveDynamicstoOutput        = 1; % Floating Dynamics are assigned to Output Struct and saved 
...
input.PatternY = {[0 42  42  -42 -42];[0 0 38.97 38.97 0  -38.97 -38.97]}; % Pattern points Y axis (in meters). Each line of the cell is a pattern
input.PatternZ = {[0 -42  42  -42  42];[0 45.00 22.50 -22.50 -45.00 -22.50 22.50]}; % Pattern points Z axis (in meters). Each line of the cell is a pattern
...
input.timestep_pat_vec      = {[5] [10]}; %Time step of the total pattern. Sampling rate of total pattern should be that npoins*timestep_meas<=timestep_pat(s)(in each combination of times this must be valid). Add one vector for each pattern
input.timeStep_Measurements = {[1 0] [1]}; %Time step between each single measured point. Add one vector for each pattern [s]
...
input.resampling_factor = 50; % Amount of desired resampling for outputs in Turbsim and PyConTurb used with flag_resampling
...
input.ReconstructionMethod = 'simple'; % 'simple': v=0,w=0 assumption %'global_projection': Pseudo-inverse method.
...
%Directory of Lidar Dynamics to be considered in floating Simulation
...
% Name of Dynamic Channels present in Dynamics variable. IMPORTANT: Keep order of this name cell,  {'Pitch'; 'Roll'; 'Yaw'; 'x_vel' ; 'y_vel'; 'z_vel'; 'x_trans_L';'y_trans_L';'z_trans_L'; 'Time'}
% Units: Angles in [deg], translations in [m] and velocities in [m/s] 
% all channels must be available in the dynamics input with same length. If dynamics channels are not available populate with zeros
% Naming Convention: Dynamics.Channelname eg. Dynamics.Pitch
input.channels.channelnames  = {'Pitch'; 'Roll'; 'Yaw'; 'x_vel' ; 'y_vel'; 'z_vel'; 'x_trans_L';'y_trans_L';'z_trans_L'; 'Time'}; % This channels represent the dynamics of the lidar system not the floater
% flags for DOF consideration
input.DOF.rot_trans = 1; % if  0: 'Pitch'; 'Roll'; 'Yaw';'x_trans';'y_trans_L';'z_trans_L'; are overwritten with zero. Translational and rotational displacement are switched on/off
input.DOF.vel  = 1; % if  0: 'x_vel' ; 'y_vel'; 'z_vel' are overwritten with zero. Translational velocities are switched on/off

%% Wind turbine parameters

input.rotor_radius = 45; % Radius of the Rotor [m]
input.Zh           = 81;  % Hub Height [m]
...
%% Plotting options
% These option are only relevant if input.plot_fullWF_Slices are switched on
input.points_plot_WF_timeseries = [42 25 0;-42 25 0];
